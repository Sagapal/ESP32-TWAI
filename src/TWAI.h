...
// Used for filter shifting such that only ID bits are checked and not data.
#define CAN_EXTD_ID_MASK                0x1FFFFFFF
...
class ESP32TWAI
{
  public:
    ...
    ESP32TWAI(int crxPin = 4, int ctxPin = 5, long baudrate = 500000, twai_mode_t mode = TWAI_MODE_NORMAL, bool autorecover = true, uint8_t rx_queue_length = 5, uint8_t tx_queue_length = 5, uint32_t acceptance_filter = 0, uint32_t filter_mask = 0, bool extended = false);
    ~ESP32TWAI();

    static ESP32TWAI *instance();
    static void initialize(int crxPin = 4, int ctxPin = 5, long baudrate = 500000, twai_mode_t mode = TWAI_MODE_NORMAL, bool autorecover = true, uint8_t rx_queue_length = 5, uint8_t tx_queue_length = 5, uint32_t acceptance_filter = 0, uint32_t filter_mask = 0, bool extended = false);
